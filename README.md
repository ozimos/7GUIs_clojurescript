...
* [`src/cljs/ozimos/gui7/`](src/cljs/ozimos/gui7/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/ozimos/gui7/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/ozimos/gui7/`](test/cljs/ozimos/gui7/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `gui7` project root directory
...
The `debug?` variable in [`config.cljs`](src/ozimos/gui7/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns ozimos.gui7.example
  (:require [ozimos.gui7.config :as config])
